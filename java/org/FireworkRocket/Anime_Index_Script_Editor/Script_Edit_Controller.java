package org.FireworkRocket.Anime_Index_Script_Editor;
...
import org.FireworkRocket.Anime_Index_Data_Manager.TXT_File_Data_Manager;

public class Script_Edit_Controller {
    @FXML
    ...
    void saveadvance() {
        if (result != null && ta.getLength() > 0) {
            ...
            TXT_File_Data_Manager.WriteFile(result, ta.getText());
    ...
    void onOpenMenu(ActionEvent event) {
        ...
        if (result != null) {
            ta.setText(TXT_File_Data_Manager.ReadFile(result));
        }
    }
            ...
            if (!tf1.getText().isEmpty()) {
                if (textString.contains(tfString)) {
                    if (startIndex == -1) {// not found
                        Alert alert1 = new Alert(Alert.AlertType.WARNING);
                        alert1.titleProperty().set("提示");
                        alert1.headerTextProperty().set("已经找不到相关内容！");
                        alert1.show();
                    }
                    startIndex = ta.getText().indexOf(tf1.getText(), startIndex);
                    if (startIndex >= 0 && startIndex < ta.getText().length()) {
                ...
                Alert alert1 = new Alert(Alert.AlertType.WARNING);
                alert1.titleProperty().set("Error");
                alert1.headerTextProperty().set("输入为空");
                alert1.show();
            }
        });
    }
    ...
    void onTypefaceMenu(ActionEvent event) throws IOException {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("../../../../resources/org/FireworkRocket/Script_Editor/Typeface.fxml"));
